# 📋 PLANO: Campo Acréscimo Automático com Registro no Banco (CORRETO)
...
Salvo: Sim, no banco com tipo (DIFERENCA/ACRESCIMO/QUITADO)

Se valor > 0  → Campo mostra diferença positiva (falta)
Se valor < 0  → Campo mostra acréscimo negativo (pagou a mais)
...
│  └─ Exemplo: +4.219,89 ❌ DIFERENÇA
│
├─ Se < 0  → Mostra em AMARELO (pagou a mais)
│  └─ Exemplo: -780,11 ⭐ ACRESCIMO
│
...
ALTER TABLE lancamentos ADD COLUMN acrescimo_tipo ENUM(
  'DIFERENCA',    -- Faltou pagar (positivo)
  'ACRESCIMO',    -- Pagou a mais (negativo)
...
4. Tipo = DIFERENÇA (porque > 0, faltou pagar)
```
...
INSERT INTO lancamentos (...) VALUES (
  ...
  acrescimo_tipo: 'DIFERENCA'← Tipo: faltou pagar
);
```

### **Exemplo 3: ACRESCIMO (Pagou a Mais)**
```sql
INSERT INTO lancamentos (...) VALUES (
  valor_recolher: 19.219,89,
  ...
  acrescimo_tipo: 'ACRESCIMO'← Tipo: pagou a mais
);
```
...
export function calcularAcrescimoAuto(
  ...
  acrescimo: number;
  tipo: 'DIFERENCA' | 'ACRESCIMO' | 'QUITADO';
  percentual: number;
  mensagem: string;
} {
  ...
  let tipo: 'DIFERENCA' | 'ACRESCIMO' | 'QUITADO';
  let mensagem: string;
...
<div className={`
  ...
  ${resultadoAcrescimo?.tipo === 'DIFERENCA' ? 'bg-red-50 border-red-200' : ''}
  ${resultadoAcrescimo?.tipo === 'ACRESCIMO' ? 'bg-yellow-50 border-yellow-200' : ''}
`}>
...
- [ ] Testar com 3 cenários (quitado, diferença, acrescimo)
...
| Faltou pagar | +4.219,89 | DIFERENÇA | 🔴 Vermelho | Faltam R$ |
| Pagou a mais | -780,11 | ACRESCIMO | 🟡 Amarelo | Recolheu a mais |

---

## 🎯 EXEMPLO COMPLETO

### **Caso 1: Recolhimento Parcial (DIFERENÇA)**
```
ENTRADA:
├─ Base: R$ 137.284,80
├─ Alíquota: 14%
...
### **Caso 2: Recolhimento Acima (ACRESCIMO)**
```
ENTRADA:
├─ Base: R$ 137.284,80
├─ Alíquota: 14%
...
├─ Valor Esperado = 137.284,80 × 14 ÷ 100 = R$ 19.219,89
├─ Total a Recolher = 19.219,89 + 0,00 = R$ 19.219,89
├─ Acréscimo = 19.219,89 - 19.219,89 = R$ 0,00
├─ Percentual = 0%
└─ Tipo = QUITADO

SAÍDA:
├─ Campo Acréscimo: 0,00
├─ Cor: 🟢 Verde
├─ Tipo: QUITADO
└─ Mensagem: "Valor exato - Quitado"
